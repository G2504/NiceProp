...
class App(tk.Frame):
    ...
    def run(self):
        ...
        if (self.settings['process'] == 'expansion') or (self.settings['process'] == 'compression'):
            ...
            throat_w  = np.array([2.70312,  2.70312,  2.70312])/1000 # mm
            chord     = np.array([36.0283, 36.0283, 36.0283])/1000 # mm
            blade_h   = np.array([5.407,  5.407, 5.407])/1000 # mm
            n_passages= np.array([7,     7,     7    ])
            ...
            print("  Average value(s) of Prandtl number:                      " + str(np.mean(flow.Pr_vec,axis=1)))
            print("  Average value(s) of ratio of specific heats (ideal gas): " + str(np.mean(flow.gamma_id,axis=1)))
            ...
            with open(os.path.join(dir,self.settings['labels'][-1]+".txt"), "w") as file:
                ...
                print("  Average value(s) of fundamental derivative of gas dyn.:  " + str(np.round(np.mean(flow.FundDerGamma,axis=1),3)),file=file)
                ...
                print("  Average value(s) of Prandtl number:                      " + str(np.mean(flow.Pr_vec,axis=1)),file=file)
                print("  Average value(s) of ratio of specific heats (ideal gas): " + str(np.mean(flow.gamma_id,axis=1)),file=file)

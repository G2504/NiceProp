...
class IsentropicFlowModel:
    ...
    def __init__(self, thermo, settings):
        ...
        self.gamma_id = np.zeros((len(self.thermo.Tt_in), self.thermo.samples))
        ...
        self.Pr_vec = np.zeros((len(self.thermo.Tt_in), self.thermo.samples))
    ...
    def IdealProcess(self):
        ...
        for ii in range(len(self.thermo.Tt_in)):
            ...
            for jj in range(self.thermo.samples):
                ...
                self.cp0_id = CoolProp.CoolProp.PropsSI('CP0MASS', "T", self.T_vec[ii,jj], "P", self.P_vec[ii,jj], self.thermo.fluid)
                self.R = CoolProp.CoolProp.PropsSI('gas_constant', self.thermo.fluid) / self.thermo.EoS.molar_mass()
                self.gamma_id[ii, jj] = self.cp0_id / (self.cp0_id - self.thermo.R)
                self.mu_vec[ii, jj] = self.thermo.EoS.viscosity()
                self.Pr_vec[ii, jj] = self.thermo.EoS.Prandtl()
